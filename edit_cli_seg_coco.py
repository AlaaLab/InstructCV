...
def inference_seg_coco(resolution, steps, vae_ckpt, split, config, eval, test_txt_path, single_test,
    ...
    for image_id in img_info:
        ...
        for cid in img_info[image_id]:
            ...
            img_path            = os.path.join(input, 'val2017/{}.jpg'.format(img_id))

        ge_path                 = os.path.join(output, img_id + "_" + cname)
        ...
        with torch.no_grad(), autocast("cuda"), model.ema_scope():
            ...
            for seg in img_info[image_id][cid]['segmentation']:
                for s in seg:
                    ...
                    seg_img = Image.fromarray(cv2.cvtColor(gt,cv2.COLOR_BGR2RGB)).convert("RGB") #gt label
